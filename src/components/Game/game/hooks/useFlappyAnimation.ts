...
const useFlappyAnimation = (config: IConfig) => {
  const { bird, foreground, background, pipeNorth, pipeSouth, go } = config.assets
  ...
  const hitDistance = pipeNorth.height + gap + 5; // This line uses the gap value
  ...
  const draw = (context: CanvasRenderingContext2D) => {
    ...
    if (gameOver.current) {
      delay.current++
      context.drawImage(go, gameOverX * 2.2, gameOverY)
