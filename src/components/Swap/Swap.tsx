...
const Swap = () => {
    ...
    const [swapInDirection, setSwapInDirection] = useState<boolean>(false); // IN: FWENDY to SOL; OUT: SOL to FWENDY
    ...
    useEffect(() => {
        async function getHelper() {
            const helper_ds = new DSHelper()
            await helper_ds.getSolanaInfo().then((r: IPair[]) => {
                let wrapSOL = r[0]
                setSolPrice(wrapSOL.priceUsd ?? '0')
            })
        }
    ...
    useEffect(() => {
        ...
        getInitialRate();
    }, [publicKey, raySolPoolKey, swapInDirection, input, slippage, connection]);
    ...
    return (
        <Box sx={(theme) => ({border: `2px solid ${theme.palette.mode === 'light' ? customColors.primaryDark : theme.palette.primary.main}`, maxWidth: 600, margin: 'auto', padding: 3, borderRadius: 2, boxShadow: 3, bgcolor: theme.palette.mode === 'light' ? theme.palette.primary.dark : 'background.paper', mt: 20 })}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                >
                    ...
                    <Button color="secondary" variant="contained" onClick={() => handlePercentageClick(25)}>25%</Button>
                ...
                >
                    ...
                    <Typography variant='body1' color="secondary">
                        {`${swapInDirection === true ? solBalance.toLocaleString('en', { maximumFractionDigits: 5 }) : rayBalance.toLocaleString('en', { maximumFractionDigits: 0 })} ${swapInDirection === true ? 'SOL' : 'FWENDY'}`}
                    </Typography>
                    <Box sx={{ flexGrow: 1 }} /> {/* This will push buttons to the end */}
                    <Modal
                    ...
                    >
                        <Box sx={(theme) => ({
                            ...
                            <Box sx={{ display: 'flex', flexDirection: 'row', width: '100%', gap: 1, alignItems: 'flex-start' }}>
                                <Typography sx={(theme) => ({ color: theme.palette.mode === 'light' ? 'black' : 'primary', mt: 1, mb: 0 })} variant="body1">Pro FWENDY Trader</Typography>
                ...
                <TextField
                    InputProps={{
                        endAdornment: <InputAdornment position="end">{`${swapInDirection === true ? 'SOL' : 'FWENDY'}`}</InputAdornment>,
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%' }}>
                    <Button
                        variant="outlined"
                        startIcon={<SwapHorizIcon sx={(theme) => ({
                            color: theme.palette.mode === 'light' ? customColors.primaryDark : theme.palette.primary.main, ml: 1
                        })} />}
                        sx={(theme) => ({border: `2px solid ${theme.palette.mode === 'light' ? customColors.primaryDark : theme.palette.primary.main}`, alignSelf: 'center' })}
                        onClick={() => {
                ...
                <TextField
                    value={output}
                    InputProps={{
                        endAdornment: <InputAdornment position="end">{`${swapInDirection === false ? 'SOL' : 'FWENDY'}`}</InputAdornment>,
                        readOnly: true,
                        sx: { bgcolor: 'background.default' }
                    }}
                ...
                >
                    <Chip label={swapInDirection === false ? `= ~${(Number(output) * Number(solPrice)).toLocaleString('en', { maximumFractionDigits: 1 })}$` : `= ~${(Number(input) * Number(solPrice)).toLocaleString('en', { maximumFractionDigits: 1 })}$`} color={'info'} />
                </Box>
                ...
                >
                    ...
                    <Typography variant='body1' color="secondary">
                        {`${swapInDirection === true ? rayBalance.toLocaleString("en", { maximumFractionDigits: 0 }) : solBalance.toLocaleString("en", { maximumFractionDigits: 5 })} ${swapInDirection === true ? 'FWENDY' : 'SOL'}`}
                    </Typography>
                    <Box sx={{ flexGrow: 1 }} /> {/* This will push buttons to the end */}
                </Box>
                ...
                >
                    <Box sx={{ display: 'flex', flexDirection: 'row', maxWidth: 600, minWidth: 'auto', justifyContent: 'space-between', width: '100%', padding: '0 16px', mt: 3 }}>
                        <Typography
                            sx={(theme) => ({
                                color: theme.palette.mode === 'light' ? customColors.primaryDark : theme.palette.secondary.dark,
                            })}
                            variant="body1"
                        >
                            Minimum Received
                        ...
                        <Typography
                            sx={(theme) => ({
                                color: theme.palette.mode === 'light' ? customColors.primaryDark : theme.palette.primary.dark,
                            })}
                            variant="body1"
                        >
                ...
                <Button
                    variant="contained"
                    sx={(theme) => ({
                        bgcolor: theme.palette.mode === 'light' ? customColors.primaryDark : theme.palette.primary.main
                    })}
                    endIcon={<ArrowDownwardIcon sx={(theme) => ({
                        color: theme.palette.mode === 'light' ? '#000' : customColors.primaryDark,
                    })}/>}
                    onClick={handleSwap}
